...
function removeYogaCard() {
    ...
    const cardTitle = 'Outfits de Yoga y Gimnasio loja';
    const cardStart = catalogContent.indexOf(cardTitle);
    ...
    // Buscar el inicio de la tarjeta (el div.product-card más cercano hacia atrás)
    const cardDivStart = catalogContent.lastIndexOf('<div class="product-card"', cardStart);
    
    if (cardDivStart === -1) {
        console.log('No se pudo encontrar el inicio de la tarjeta.');
        return;
    }
    
    // Buscar el final de la tarjeta (el cierre del div.product-card correspondiente)
    let openDivs = 1;
    ...
    if (cardDivEnd > 0) {
        ...
        fs.writeFileSync(catalogPath + '.backup11', catalogContent);
        
        // Eliminar la tarjeta
        const newContent = catalogContent.substring(0, cardDivStart).trim() + 
                         catalogContent.substring(cardDivEnd).trim();
